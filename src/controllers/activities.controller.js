...
export const listActivities = asyncHandler(async (req, res) => {
  ...
  const [rows] = await pool.query(
    `SELECT id, bench_level_id, activity_type, title, description, activity_date
     FROM activities
...
export const createActivity = asyncHandler(async (req, res) => {
  const { siteId, benchLevelId, activityType, title, description, activityDate } = req.body || {}
  if (!siteId || !activityType || !title || !activityDate) {
    throw ApiError.badRequest('ต้องระบุ siteId, activityType, title, activityDate')
  }

  const [result] = await pool.query(
    `INSERT INTO activities (site_id, bench_level_id, activity_type, title, description, activity_date)
     VALUES (:siteId, :benchLevelId, :activityType, :title, :description, :activityDate)`,
    { siteId, benchLevelId: benchLevelId || null, activityType, title, description: description || null, activityDate }
  )
  res.status(201).json({ id: result.insertId })
})

export const updateActivity = asyncHandler(async (req, res) => {
  const { benchLevelId, activityType, title, description, activityDate } = req.body || {}
